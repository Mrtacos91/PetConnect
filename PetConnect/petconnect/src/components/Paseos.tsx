...
const Foods: React.FC = () => {
          ...
          if (notification) {
            showNotification(
              ...
              notification.message ||
                "¡Es hora de realizar una actividad con tu mascota!"
            );
            ...
            if (
              ...
              navigator.serviceWorker.ready
                .then((registration) => {
                  registration.showNotification(`¡Hora de actividad!`, {
                    body:
                      notification.message ||
                      "¡Es hora de realizar una actividad con tu mascota!",
                ...
                .catch(() => {
                  // Fallback clásico
                  new Notification(`¡Hora de actividad!`, {
                    body:
                      notification.message ||
                      "¡Es hora de realizar una actividad con tu mascota!",
                    icon: "/public/images/logo_gradient.png",
    ...
    const checkAlarms = async () => {
      ...
      activeAlarms.forEach(async (alarm) => {
        ...
        await sendFoodNotification(
          userId,
          `¡Es hora de realizar una actividad con ${
            alarm.name
  ...
  const saveAlarmToDatabase = async (alarm: FoodAlarm) => {
    ...
    try {
      ...
      showNotification("success", "¡Recordatorio de actividad programado!");

      // Enviar notificación de confirmación
      if (userId) {
        await sendFoodNotification(
          userId,
          `Se ha programado una actividad para ${
            alarm.name
          } a las ${alarm.time?.format("h:mm A")}`
        );
      }
  ...
  const scheduleAlarm = async (alarmIndex: number) => {
    ...
    try {
      ...
      if (updatedAlarm) {
        ...
        showNotification(
          "success",
          `Actividad para "${alarm.name}" programada para ${alarm.days.join(
            ", "
          )} a las ${alarm.time.format("h:mm A")}`
        );
      }
      ...
      showNotification(
        "error",
        "Error al programar la actividad. Por favor, inténtalo de nuevo."
      );
    }
  };
  ...
  const testAlarm = async (alarmIndex: number) => {
    ...
    try {
      ...
      const hora = alarm.time
        ...
        : "hora no definida";
      const testMessage = `¡Recordatorio de prueba! Es hora de realizar una actividad con ${alarm.name} a las ${hora}. ${dias}`;
      ...
      if ("Notification" in window && Notification.permission === "granted") {
        if ("serviceWorker" in navigator) {
          try {
            ...
            await registration.showNotification(
              `¡Hora de actividad, ${alarm.name}!`,
              {
                body: `¡Es hora de realizar una actividad con ${alarm.name} a las ${hora}! ${dias}`,
                icon: "/public/images/logo_gradient.png",
                badge: "/public/images/logo_gradient.png",
                data: { url: window.location.origin },
              }
        ...
        try {
          new Notification(`¡Hora de actividad, ${alarm.name}!`, {
            body: `¡Es hora de realizar una actividad con ${alarm.name} a las ${hora}! ${dias}`,
            icon: "/public/images/logo_gradient.png",
            badge: "/public/images/logo_gradient.png",
            data: { url: window.location.origin },
  ...
  return (
    <div className="FOODS-section">
      ...
      {isLoading ? (
        ...
        <>
          <h2 className="FOODS-title"> Programar Actividades </h2>
          <div>
